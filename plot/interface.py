...
def relative_parameter_confidence(parameter_set_nr, kind='WOA_WLS', path='/tmp'):
    ...
    ## load value
    parameter_dirname = MODEL_PARAMETERS_SET_DIRNAME.format(parameter_set_nr)
    parameter_dir = os.path.join(MODEL_OUTPUT_DIR, MODEL_TIME_STEP_DIRNAME.format(1), parameter_dirname)
    parameter_value_file = os.path.join(parameter_dir, MODEL_PARAMETERS_FILENAME)
...
def model_confidence(parameter_set_nr, kind='WOA_WLS', path='/tmp', v_max=[None, None], time_dim_confidence=12, time_dim_df=12, average_in_time=False):
    ...
    ## load value
    parameter_set_dirname = MODEL_PARAMETERS_SET_DIRNAME.format(parameter_set_nr)
    f_file = os.path.join(MODEL_OUTPUT_DIR, MODEL_TIME_STEP_DIRNAME.format(1), parameter_set_dirname, CACHE_DIRNAME, kind, MODEL_CONFIDENCE_FILENAME.format(time_dim_confidence=time_dim_confidence, time_dim_df=time_dim_df))
    f = np.load(f_file)
    if average_in_time:
        f = f.mean(axis=1)
    assert len(f) == 2
