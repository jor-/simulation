...
class Cache():
    ...
    def __init__(self, base_dir, measurements_object, model_options=None, model_job_options=None, include_initial_concentrations_factor_to_model_parameters=True):
        ...
        self.default_initial_concentrations = np.asarray(self.model.model_options.initial_concentration_options.concentrations)
    ...
    def model_parameters(self):
        ...
        if self.include_initial_concentrations_factor_to_model_parameters:
            ...
            initial_base_concentrations = self.default_initial_concentrations
            factors = initial_concentrations / initial_base_concentrations
            assert np.allclose(factors[:-1], factors[1:])
            concentration_parameter = factors.mean()
    ...
    def model_parameters(self, model_parameters):
        ...
        if len(model_parameters) == model_parameters_len:
            ...
            self.model_options.initial_concentration_options.concentrations = self.default_initial_concentrations * model_parameters[-1]
    ...
    def _value_from_file_cache(self, filename, calculate_method, derivative_used=True, derivative_accuracy_order=None, save_as_txt=True, save_as_np=False):
        filename = self._filename(filename)
        file = self.model._cache.get_file(filename, derivative_used=derivative_used, derivative_accuracy_order=derivative_accuracy_order)
        value = self.model._cache.get_value(file, calculate_method, save_as_txt=save_as_txt, save_as_np=save_as_np)
        return value

    def _value_in_file_cache(self, filename, derivative_used=True, derivative_accuracy_order=None):
        filename = self._filename(filename)
        file = self.model._cache.get_file(filename, derivative_used=derivative_used, derivative_accuracy_order=derivative_accuracy_order)
