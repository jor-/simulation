...
def database_array_file(cost_function_name, model_name, time_step,
    ...
    model_options = simulation.model.options.ModelOptions({'model_name': model_name, 'time_step': time_step})
    ...
    base_dir = os.path.join(model_dir, simulation.model.constants.DATABASE_TIME_STEP_DIRNAME.format(time_step), 'cf_values')
    filename = 'cf_values.npy'
    array_file = cost_function._filename(filename, base_dir=base_dir)
    return array_file


def update_db(cost_function_name, model_name, time_step,
              min_standard_deviations=None, min_measurements_correlations=None, max_box_distance_to_water=None,
              overwrite=True):

    array_file = database_array_file(
        cost_function_name, model_name, time_step,
        min_standard_deviations=min_standard_deviations,
        min_measurements_correlations=min_measurements_correlations,
        max_box_distance_to_water=max_box_distance_to_water)
    ...
    for cf in simulation.optimization.cost_function.iterator([cost_function], model_names=[model_name], time_steps=[time_step]):
        if cf.f_available():
            parameters = cf.model.parameters
            concentrations = cf.model.initial_constant_concentrations
            key = np.concatenate([parameters, concentrations])
            f_value = cf.f()
...
def _main():
    ...
    parser.add_argument('time_step', type=int, choices=simulation.model.constants.METOS_TIME_STEPS, help='The time step that the model should use.')
    ...
    with util.logging.Logger(level=args.debug_level):
        update_db(
            args.cost_function_name, args.model_name, args.time_step,
