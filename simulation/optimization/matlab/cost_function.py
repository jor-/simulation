if __name__ == "__main__":
    ...
    with util.logging.Logger(log_file=log_file, disp_stdout=log_file is None):
        ...
        if (eval_function_value and not cf.f_available()) or (eval_grad_value and not cf.df_available()):
            ...
            with simulation.optimization.job.CostFunctionJob(output_dir, cf_kind, model_options, job_options=prepare_job_options(), max_box_distance_to_water=max_box_distance_to_water, min_measurements_correlation=min_measurements_correlation, eval_f=eval_function_value, eval_df=eval_grad_value) as cf_job:
                cf_job.start()
